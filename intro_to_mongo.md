...
[Our Cheatsheet](https://git.generalassemb.ly/seir-9-21/student-resources/wiki/Mongo-Cheatsheet)
...
We already installed Mongo, if you need to reinstall, [follow the directions here](https://git.generalassemb.ly/seir-9-21/student-resources/blob/master/2_full_stack_dev/w05d05/instructor_notes/install_mongo.md)
...
We can add a field. Kelly has no state, let's give her a state
...
And we should see that Kelly now has a state, so we don't have to query for the field that we want to change, we can query for any match.
